...
namespace Utilities
{
    public static class Text
    {
		...
		public static string trataTags(string value, string[] excecoes)
		{
			string temp = value.Replace("<", "&lt;");
			foreach (string excecao in excecoes)
			{
				temp = temp.Replace("&lt;" + excecao, "<" + excecao).Replace("&lt;/" + excecao, "</" + excecao);
			}
			return temp;
		}
		...
		public static string TrataValorCSV(dynamic data)
		{
			String ret = String.Empty;
			try
			{
				if (data is String) data = ((String)data).Replace("\"", String.Empty).Replace("\r\n", String.Empty).Replace("\n", String.Empty).Replace("\r", String.Empty);
				if (data is Int32?) data = (data != null) ? ((Int32?)data).ToString() : String.Empty;
				if (data is Int64?) data = (data != null) ? ((Int64?)data).ToString() : String.Empty;
				if (data is Decimal?) data = (data != null) ? ((Decimal)data).ToString("0.00") : String.Empty;
				if (data is Boolean?) data = (data != null) ? (((Boolean?)data == true) ? "Sim" : "Não") : String.Empty;
				if (data is DateTime?) data = (data != null) ? ((DateTime)data).ToString("dd/MM/yyyy HH:mm:ss") : String.Empty;
				ret = data;
			}
			catch (Exception)
			{
