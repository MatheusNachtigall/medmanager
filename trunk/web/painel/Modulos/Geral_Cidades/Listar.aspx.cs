...
public partial class Manager_Modulos_Geral_Cidades_Listar : System.Web.UI.Page
{
    #region Variáveis
    private int Pagina;
    ...
    protected void rptTable_ItemDataBound(Object sender, RepeaterItemEventArgs e)
    {
        ...
        {
			CIDADE item = (CIDADE)e.Item.DataItem;
            ((Literal)e.Item.FindControl("ltlID")).Text = ((HiddenField)e.Item.FindControl("hdnID")).Value = item.CIDADE_ID.ToString();
    ...
    private void CarregarFiltro()
    {
        ...
        Session["CIDADE_Filtro"] = filtro;
    }

    private void CarregarLista()
    {
        CarregarFiltro();
        CIDADE_Ordem Ordem = (Session["CIDADE_Ordem"] != null) ? (CIDADE_Ordem)Session["CIDADE_Ordem"] : CIDADE_Ordem.NOME;
        OrdemTipo OrdemTipo = (Session["CIDADE_Ordem"] != null) ? (OrdemTipo)Session["CIDADE_Ordem_Tipo"] : OrdemTipo.Ascendente;
        Paginacao<CIDADE> pagLista = new CIDADE_Service().Listar((CIDADE)Session["CIDADE_Filtro"], this.Pagina, 10, Ordem, OrdemTipo);
	...
	{
		ddlNome.DataSource = new CIDADE_Service().Listar(CIDADE_Ordem.NOME, OrdemTipo.Ascendente);
		ddlNome.DataTextField = "NOME";
		ddlNome.DataValueField = "CIDADE_ID";
		ddlNome.DataBind();
