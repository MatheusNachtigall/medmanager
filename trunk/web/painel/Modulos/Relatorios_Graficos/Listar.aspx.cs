...
public partial class Manager_Modulos_Relatorios_Graficos_Listar : System.Web.UI.Page
{
    #region Eventos
    ...
    protected void rptTable_ItemDataBound(Object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
			...
			((Literal)e.Item.FindControl("ltlValor")).Text = (item.valor_mes == 0) ? String.Empty : ((Decimal)item.valor_mes).ToString("0.00");
			((Literal)e.Item.FindControl("ltlINSS")).Text = (item.valor_mes_inss == 0) ? String.Empty : ((Decimal)item.valor_mes_inss).ToString("0.00");
		}
    }
    #endregion
    ...
    private void CarregarFiltro()
    {
		...
		filtro.DATA_FIM = String.IsNullOrEmpty(ddlDataFim.SelectedValue) ? DateTime.Now.AddMonths(1).AddMinutes(-1) : DateTime.Parse(ddlDataFim.SelectedValue).AddMonths(1).AddMinutes(-1);
    ...
    private void CarregarLista()
    {
		...
		List<ListaRelatorio> lista = new _APP_ADO().GET_RELATORIO_PAGAMENTO_DATA( (RelatorioFiltro)Session["RELATORIO_Filtro"] );

		hdDados.Value = JsonConvert.SerializeObject(lista);
		hdNome.Value = "Pagamento";
	...
	private void CarregarCombos()
	{
		...
		filtro.DATA_FIM = DateTime.Now.AddMonths(1).AddMinutes(-1);
