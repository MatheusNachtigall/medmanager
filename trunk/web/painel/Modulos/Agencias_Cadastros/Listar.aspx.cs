...
public partial class Manager_Modulos_Agencias_Cadastros_Listar : System.Web.UI.Page
{
    #region Variáveis
    private int Pagina;
    ...
    protected void rptTable_ItemDataBound(Object sender, RepeaterItemEventArgs e)
    {
        ...
        {
            AGENCIA item = (AGENCIA)e.Item.DataItem;
            ((Literal)e.Item.FindControl("ltlID")).Text = ((HiddenField)e.Item.FindControl("hdnID")).Value = item.AGENCIA_ID.ToString();
    ...
    private void CarregarFiltro()
    {
        ...
        Session["AGENCIA_Filtro"] = filtro;
    }

    private void CarregarLista()
    {
        CarregarFiltro();
        AGENCIA_Ordem Ordem = (Session["AGENCIA_Ordem"] != null) ? (AGENCIA_Ordem)Session["AGENCIA_Ordem"] : AGENCIA_Ordem.NOME;
        OrdemTipo OrdemTipo = (Session["AGENCIA_Ordem"] != null) ? (OrdemTipo)Session["AGENCIA_Ordem_Tipo"] : OrdemTipo.Ascendente;
        Paginacao<AGENCIA> pagLista = new AGENCIA_Service().Listar((AGENCIA)Session["AGENCIA_Filtro"], this.Pagina, 10, Ordem, OrdemTipo);
